...
void create_node_map(int n_shader, int n_mem, int n_cpu, int size, int use_map)
{
    char temp[PATH_MAX];
    std::string path;
    path = getcwd(temp, sizeof(temp)) ? std::string(temp) : std::string("");
    path += "/layout_conf.file";
    std::fstream layout_conf_file(path.c_str(), std::fstream::in);
    if (!layout_conf_file.is_open()) {
        std::cout << "Error opening layout_conf_file" << std::endl;
        exit(1);
    }
    node_map = (int*)malloc((size) * sizeof(int));
    int map_pos = 0;
    std::vector<int> core_counts;
    core_counts.push_back(n_shader);
    core_counts.push_back(n_cpu);
    core_counts.push_back(n_mem);
    ...
    if (use_map) {
        ...
        { // for CPU+GPU mesh topology 8 MC, 14 CPU, 14*2 GPU
            ...
            if (map_pos != size) {
                std::cout << "Incorrect layout configuration: incorrect number of cores. " << map_pos << std::endl;
            }
            break;
        }
        case 64:
